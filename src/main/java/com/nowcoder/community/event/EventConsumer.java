...
public class EventConsumer implements CommunityConstant {
    ...
    public void handleShareMessage(ConsumerRecord record){
        ...
        try {
            Runtime.getRuntime().exec(cmd);//这个命令生成图片是耗时的，后面的命令要先于这句话执行
            logger.info("生成长图成功："+cmd);//这行代码要先于上一行代码执行完
        } catch (IOException e) {
            logger.error("生成长图失败："+e.getMessage());
        }

        //将生成的图片传到云服务器，要等待图片生成完才能上传，人为启动一个定时器，每隔一会查看是否生成图片
        // 启用定时器,监视该图片,一旦生成了,则上传至七牛云.
        UploadTask task = new UploadTask(fileName, suffix);//实例化线程体
        Future future = taskScheduler.scheduleAtFixedRate(task, 500);//启动定时器
        task.setFuture(future);//停止定时器
    }

    class UploadTask implements Runnable {//线程体
        ...
        public void run() {
            ...
            String path = wkImageStorage + "/" + fileName + suffix;//本地存放的路径
            ...
            if (file.exists()) {
                ...
                try {
                    ...
                    // 处理响应结果
                    JSONObject json = JSONObject.parseObject(response.bodyString());//将得到的json字符串转换为json对象
